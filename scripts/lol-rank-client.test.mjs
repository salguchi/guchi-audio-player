...
const snapshot = (extra = {}) => ({schemaVersion:1, riotId:'Re 제로부터시작하는다이아생활#탑레마스터', queue:'RANKED_SOLO_5x5', scope:'opgg-displayed-solo-current', status:'ranked', tier:'MASTER', division:null, rankLabel:'마스터', lp:90, checkedAt:new Date().toISOString(), ...extra});
...
test('frontend accepts newer LP decreases and demotion instead of retaining a peak', async () => {
  ...
  await tick();
  state.data = snapshot({lp:75, checkedAt:new Date(now - 1000).toISOString()});
  state.refresh();
  await tick();
...
test('unranked hides the icon and a later ranked snapshot restores it', async () => {
  const now = Date.now();
  const state = browser(snapshot({status:'unranked', tier:'UNRANKED', division:null, rankLabel:'언랭크', lp:null, checkedAt:new Date(now - 1000).toISOString()}));
  await tick();
